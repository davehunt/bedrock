...
.. code-block:: bash
    ...
    $ bin/peep.py install -r requirements/test_functional.txt
...
.. code-block:: bash
    ...
    $ py.test --driver Firefox --html results.html -n auto tests/functional/
...
be reported in ``results.html`` and tests will run in parallel according to the
...
.. code-block:: bash
    ...
    $ py.test --driver Firefox --html results.html -n auto tests/functional/test_newsletter.py
...
.. code-block:: bash
    ...
    $ py.test --base-url https://www-dev.allizom.org --driver Firefox --html results.html -n auto tests/functional/
...
tests by setting the ``SELENIUM_CAPTURE_DEBUG`` environment variable to any
value.
